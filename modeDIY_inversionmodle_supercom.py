...
class signal_3Dvolume_simu():
    ...
    def get_simulatoin_signal_correlation_function(self, subarray1, subarray2, incoherent=2**3, flag_dvl0=False):
        ...
        out = {'fft_xcf':fft_xcf, 'doppler_freq':frq, 'xcf':xcf, 'lag_time_all':lag_time_all, 'lag_time_incoherent':lag_time_incoherent, 'name_list':label_mu}
            #    'mu_label':label_mu, 'range':self.radar_range, \
            #    'obsmode':self.obsmode, 'sampling_time_len':time_len, 'incoherent':incoherent}
        return out


    def get_unit_simulation_signal_correlation_function(self, incoherent_u=32, flag_dvl0=False):
        ...
        #             namelist_u.append(out.get('name_list'))
                    ...
                    # print(s1,s2)
                    # print(namelist_u)
        #test 3 subarrays:F2,F3,F4(0,1,2)
        testcomb = [[0,1],[0,2],[1,2]]
        for i,j in testcomb:
            out = self.get_simulatoin_signal_correlation_function(subarray1=i,subarray2=j,incoherent=incoherent_u, flag_dvl0=flag_dvl0)
            fft_xcf_unit.append(out.get("fft_xcf")) 
            xcf_unit.append(out.get("xcf"))
            namelist_u.append(out.get('name_list'))
        ...
        outunit = {'fft_xcf_unit':fft_xcf_unit, 'doppler_freq':frq, \
               ...
               'sampling_time_len':time_len, 'incoherent':incoherent_u}
        return outunit
...
class radar_inversion():
    ...
    def inversion_vr_vv_vh(self, fftR, xcf_field, lag_time, xi_field, sampling_time_len, initial_param=None, optimize="leastsq"):
        xcf_window = window_correlation(lag=lag_time, width_pass=sampling_time_len)       
        unit_len = fftR.shape[0]                            
        # print('fftR',fftR.shape)
        # print('unit_len',unit_len)
        label_fc = []                                       
        if (initial_param is None):
            high = np.max(np.abs(fftR))
            initial_param = np.array( [ 1, 10, 0, high ] )                   
        else:
            initial_param = initial_param                                                   
        method = estimate_get_vr_vv_vh(fftR=fftR, GG_xi=xcf_field, WW=xcf_window, \
        ...
        if optimize=="leastsq":
            ...
            fftFGW = method.get_fftFGW(param=least_sq[0])
            error = method.get_residual(param=least_sq[0]).reshape(fftFGW.shape)
         ...
         #   vvel = omega * self.lamda / (4*np.pi*np.cos(self.zenith/180*np.pi))         # [m/s]
            ...
            vvel = omega * self.lamda / (4*np.pi*np.cos(self.zenith/180*np.pi))        # [m/s]
            hvel = least_sq.x[2]                                                       # [m/s]
            phase0 = least_sq.x[3]                                                     #randar [rad]
            scale = least_sq.x[4]                                                      
            info_opt = least_sq
        else:
        ...
        out ={"param":param, "sq_error":sq_error, "fft_FGW":fftFGW}
#         out ={"sq_error":sq_error}
        return out

class estimate_get_vr_vv_vh():

    def __init__(self, fftR, GG_xi, WW, lag_time, xi):
        self.fftR = np.atleast_2d(fftR)
        self.GG_xi = np.atleast_2d(GG_xi)
        self.WW = WW
        self.lag_time = lag_time
        self.xi = xi
        self.subarraysize = self.fftR.shape[0]

    def get_residual(self, param):
        fftFGW = self.get_fftFGW(param)
        res = np.abs( np.ravel( self.fftR - fftFGW ) )
    ...
    def get_fftFGW(self, param):
        fftFGW = np.zeros_like(self.fftR, dtype=complex)
